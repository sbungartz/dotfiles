...
module Brew
  ...
  class FormulaUpdates < Common
    ICON_NO_UPDATES = ":checkmark.arrow.trianglehead.counterclockwise:"
    ICON_HAS_UPDATES = ":exclamationmark.arrow.trianglehead.2.clockwise.rotate.90:"

    #prefix '🍻'
    prefix ICON_NO_UPDATES

    def run
      brew_check(printer)
      brew_update

      any_updates = formulas.size.positive? || casks.size.positive?

      if any_updates
        printer.item("#{ICON_HAS_UPDATES}↑#{formulas.size + casks.size}", dropdown: false)
      else
        printer.item("#{ICON_NO_UPDATES}")
      end
      printer.sep
